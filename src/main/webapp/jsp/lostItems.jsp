<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"
         import="com.lostfound.model.LostItem, java.util.List" %>
<%
    ...
    // Check if user is logged in (for action buttons)
...
<head>
    ...
    <title>Lost Items — College Lost &amp; Found Portal</title>
    <meta name="description" content="Browse all lost items reported on campus. Search by name, category, or location.">
...
<nav class="navbar-custom">
    <div class="container d-flex align-items-center justify-content-between flex-wrap gap-2">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
            <a href="${pageContext.request.contextPath}/lost-items"  class="nav-link-custom active-nav">Lost Items</a>
...
<div class="container py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        <div>
            <h1 class="h3 fw-800 mb-1">
                <i class="fa fa-search me-2" style="color:#fca5a5;"></i>Lost Items
            </h1>
            <p class="text-secondary-custom mb-0" style="font-size:0.88rem;">
                <%= searching ? "Search results" : "All active reports" %>
                &nbsp;·&nbsp; <strong style="color:var(--text-primary);"><%= items != null ? items.size() : 0 %></strong>
                <%= items != null && items.size() == 1 ? "item" : "items" %> found
            </p>
        </div>
        <% if (isLoggedIn) { %>
        <a href="${pageContext.request.contextPath}/report-lost" class="btn btn-danger-custom">
            <i class="fa fa-plus me-2"></i>Report Lost Item
        </a>
        <% } %>
    </div>

    <!-- Search & Filter -->
    <form id="searchForm" action="${pageContext.request.contextPath}/lost-items" method="get" class="mb-4">
        <div class="search-bar-wrap">
            <i class="fa fa-search ms-2" style="color:var(--text-muted);flex-shrink:0;"></i>
            <input type="text" name="keyword" placeholder="Search by item name, description, location..."
            ...
            <button type="submit" class="btn btn-primary-custom" style="flex-shrink:0;padding:0.5rem 1.25rem;">Search</button>
            <% if (searching) { %>
            <a href="${pageContext.request.contextPath}/lost-items" style="color:var(--text-muted);flex-shrink:0;padding:0 0.5rem;font-size:0.85rem;">
    ...
    <div class="glass-card text-center py-5">
        ...
        <h4 class="fw-700">No items found</h4>
        <p class="text-secondary-custom">
            <%= searching ? "Try different search terms or clear the filter." : "No lost items have been reported yet." %>
        </p>
        <% if (isLoggedIn) { %>
        <a href="${pageContext.request.contextPath}/report-lost" class="btn btn-danger-custom mt-2">
            <i class="fa fa-plus me-2"></i>Be the first to report
        </a>
        <% } %>
    </div>
    <% } else { %>
        ...
        <div class="col-sm-6 col-md-4 col-lg-3">
            <div class="item-card">
                ...
                <div class="item-img-wrap">
                    ...
                    <span class="badge-custom badge-<%= item.getStatus().toLowerCase() %>"
                ...
                <div class="item-body">
                    ...
                    <div class="item-meta">
                        <i class="fa fa-tag"></i><%= item.getCategory() %>
                    </div>
                    <div class="item-meta">
                        <i class="fa fa-location-dot"></i><%= item.getLocation() %>
                    </div>
                    <div class="item-meta">
                        <i class="fa fa-calendar"></i><%= item.getDateLost() %>
                    </div>
                    ...
                    <div class="item-footer">
                        ...
                        <a href="${pageContext.request.contextPath}/item-detail?type=lost&id=<%= item.getItemId() %>"
